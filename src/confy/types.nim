...
type BuildTrg * = object
  ...
  src      *:PathList      ## @field src Sequence of source files to build with. Object files (aka `.o`, etc) will be linked at the end and their path won't be adjusted.
  ...
  remotes  *:PathList      ## @field remotes Remote folders to search for files (in order), when they are not found in the main folder.
  ...
  version  *:Version       ## @field version Version string. Currently used for info reports in CLI with `BuildTrg.print()`.
  # Internals
