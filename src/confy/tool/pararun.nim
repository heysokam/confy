...
#_____________
# Code from:  |___________________________________________________________
...
proc exec *(cmds :seq[string]; numWorkers :int) :void=
  # State
  # -----
  let wq = WorkQueue(
    sem         : AsyncSemaphore.new(numWorkers),
    cmdQueue    : initDeque[string](),
    outputQueue : newAsyncQueue[tuple[
      cmd       : string,
      p         : AsyncProcess,
...
proc main=
  ...
  if paramCount() == 0:  echo &"Usage: {getAppFilename().extractFilename()} <file-with-commands-1-per-line> <numWorkers: {numWorkers}>"
  if paramCount() >= 1:  commandFile = paramStr(1)
  if paramCount() == 2:  numWorkers  = paramStr(2).parseInt()
  if paramCount() > 2:   echo &"Usage: {getAppFilename().extractFilename()} <file-with-commands-1-per-line> <numThreads: {numWorkers}>"; quit 1
